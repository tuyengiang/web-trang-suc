...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (empty($thongbao)) {
        if (move_uploaded_file($images_tmp, $path)) {
            $sql = "UPDATE products SET title='{$title}',excerpt='{$excerpt}',content='{$content}',images='{$anh}',category_id='{$category_id}',tags='{$tags}',post_type='product',is_hot='{$is_hot}',price='{$price}',price_sale='{$price_sale}',is_plus='{$is_plus}',slug='{$slug}' WHERE id_product='{$id}'";
            $query = mysqli_query($conn, $sql);
            if ($query) {
                echo "<script>alert('Sửa sản phẩm thành công !!!');</script>";
            } else {
                echo "<script>alert('Sửa sản phẩm thành thất bại !!!');</script>";
            }
        }
    }
}
...
<form method="POST" action="" enctype="multipart/form-data">
    <div class="col-md-8 col-xs-8 col-sm-12 col-xs-12">
        ...
        <div class="form-group">
            ...
            <textarea name="is_plus" id="textarea1" rows="15"><?php echo $row['is_plus']; ?></textarea>
        </div>
        <div class="form-group">
    ...
    <div class="col-md-4 col-xs-4 col-sm-12 col-xs-12">
        <div class="form-group">
            <label>Giá bán</label>
            <input type="text" name="price" required class="form-control" value="<?php echo $row['price']; ?>">
        </div>
        <div class="form-group">
            <label>Giá khuyến mại (nếu có)</label>
            <input type="text" name="price_sale" class="form-control" value="<?php echo $row['price_sale']; ?>">
        </div>
        <div class="form-group">
            <label>Sản phẩm nổi bật</label>
            <div class="">
                <label>
                    <input type="checkbox" class="js-switch" name="is_hot"
                           value="1" <?php if ($row['is_hot'] == 1): echo 'checked'; endif; ?> data-switchery="true"
                           style="display: none;">
                    Nổi bật
                </label>
            </div>
        ...
        <div class="form-group">
            <label>Từ khóa</label>
            <input type="text" name="tags" required class="form-control" value="<?php echo $row['tags']; ?>">
        </div>
        <div class="form-group">
